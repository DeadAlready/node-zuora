...
function translateError(code) {
    ...
    return (entries[c.entry] + ': ' + objects[c.object] + ',' + c.field + ' - ' + categories[c.category]);
};

function splitError(code) {
    code = code + '';
    return {
        entry: code.substr(0, 1),
        object: code.substr(1, 3),
        field: code.substr(4, 2),
        category: code.substr(6, 2)
    };
}
