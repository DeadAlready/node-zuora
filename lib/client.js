...
function ZuoraClient(opts) {
    ...
    this.log = function (lvl, message, err) {
        if(opts.log) {
            if(err) {
                opts.log[lvl](message, err);
            } else {
                opts.log[lvl](message);
            }
        }
    }
...
ZuoraClient.prototype.get = function (path, opts, callback) {
    ...
    var method = 'Zuora GET:' + path;
    this.log('debug', 'Calling ' + method);
    this.client.get(path, error.getHandler(this.log, method + ' failed', callback));
};
...
ZuoraClient.prototype.del = function (path, opts, callback) {
    ...
    var method = 'Zuora DELETE:' + path;
    this.log('debug', 'Calling ' + method);
    this.client.del(path, error.getHandler(this.log, method + ' failed', callback));
};
...
ZuoraClient.prototype.put = function (path, object, callback) {
    ...
    var method = 'Zuora PUT:' + path;
    if(object) {
        method += ' with: ' + JSON.stringify(object);
        this.log('debug', 'Calling ' + method);
        this.client.put(path, object, error.getHandler(this.log, method + ' failed', callback));
    } else {
        this.log('debug', 'Calling ' + method);
        this.client.put(path, error.getHandler(this.log, method + ' failed', callback));
    }
};
...
ZuoraClient.prototype.post = function (path, object, callback) {
    ...
    var method = 'Zuora POST:' + path;
    if(object) {
        method += ' with: ' + JSON.stringify(object);
        this.log('debug', 'Calling ' + method);
        this.client.post(path, object, error.getHandler(this.log, method + ' failed', callback));
    } else {
        this.log('debug', 'Calling ' + method);
        this.client.post(path, error.getHandler(this.log, method + ' failed', callback));
    }
};
